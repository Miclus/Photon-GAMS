...
// #include "/include/tonemapping/hatchling_oklab.glsl"
    ...
    || (defined(TONEMAP_COMPARISON) \
        && (tonemap_left == tonemap_opendrt \
            || tonemap_right == tonemap_opendrt))
#undef tonemap_opendrt
// #include "/include/tonemapping/opendt/opendrt.glsl"
#endif

#if (tonemap == tonemap_jzdt) \
    || (defined(TONEMAP_COMPARISON) \
        && (tonemap_left == tonemap_jzdt || tonemap_right == tonemap_jzdt))
#undef tonemap_jzdt
// #include "/include/tonemapping/opendt/jzdt.glsl"
#endif

#if (tonemap == tonemap_rgbdrt) \
    || (defined(TONEMAP_COMPARISON) \
        && (tonemap_left == tonemap_rgbdrt \
            || tonemap_right == tonemap_rgbdrt))
#undef tonemap_rgbdrt
#include "/include/tonemapping/rgbdrt.glsl"
#endif

#if (tonemap == tonemap_zcam_drt) \
    || (defined(TONEMAP_COMPARISON) \
        && (tonemap_left == tonemap_zcam_drt \
            || tonemap_right == tonemap_zcam_drt))
#undef tonemap_zcam_drt
#include "/include/tonemapping/zcam_drt.glsl"
#endif

#if (tonemap == tonemap_fidelityfx_lpm) \
    || (defined(TONEMAP_COMPARISON) \
        && (tonemap_left == tonemap_fidelityfx_lpm \
            || tonemap_right == tonemap_fidelityfx_lpm))
...
vec3 grade_output(vec3 rgb) {
    ...
    // Oranges
    float orange = isolate_hue(
        hsl,
        30.0,
        20.0
    ); // isolate_hue(hsl, 30.0, 20.0) // custom : 20.0, 30.0
    ...
    // Greens
    float green = isolate_hue(
        hsl,
        90.0,
        53.0
    ); // isolate_hue(hsl, 90.0, 44.0) // custom : 90.0, 53.0
...
vec3 tonemap_melon(vec3 rgb) {
    ...
    float factor
        = max_of(rgb) * MELON_TONEMAP_SHIFT_STRENGTH; // multiply by 0.15 to get
                                                      // a similar look to ACES
    ...
    rgb = mix(
        ...
        melon_hueshift(rgb),
        factor
    ); // can be removed for more neutral colors
#endif
...
#define COLORTEST_BOTTOSSON_BRIGHTNESS 2.0
    ...
    float time = frameTimeCounter * COLORTEST_BOTTOSSON_BRIGHT_SPEED
        + COLORTEST_BOTTOSSON_START_BRIGHT * 0.5;

    scene_color
        = bottosson_color_test(
              vec3(0.0),
              pos,
              time,
              COLORTEST_BOTTOSSON_BRIGHTNESS,
              COLORTEST_BOTTOSSON_AMOUNT
          )
        * display_to_working_color;
    // scene_color = vec3(aspectRatio-length(pos), clamp01(abs(pos.x)),
    // clamp01(pos.y));
#endif
...
// Tonemap plot
    ...
    if (abs(uv_scaled.x - 1.0) < 0.001 * scale) {
        scene_color = vec3(1.0, 0.0, 0.0);
    }
    if (abs(uv_scaled.y - 1.0) < 0.001 * scale) {
        scene_color = vec3(1.0, 0.0, 0.0);
    }
    if (abs(uv_scaled.y - y) < 0.001 * scale) {
        scene_color = vec3(1.0);
    }
    ...
    if (abs(uv_scaled.x - 1.0) < line) {
        scene_color = vec3(0.0);
    }
    if (abs(uv_scaled.y - 1.0) < line) {
        scene_color = vec3(0.0);
    }
    ...
    if (abs(uv_scaled.y - y[3].x) < line) {
        scene_color = y[3] * working_to_display_color;
    }

    if (abs(uv_scaled.y - y[0].r) < line || abs(uv_scaled.y - y[1].g) < line
        || abs(uv_scaled.y - y[2].b) < line) {
        scene_color = vec3(0.0);
    }

    if (abs(uv_scaled.y - y[0].r) < line) {
        scene_color += y[0] * working_to_display_color;
    }
    if (abs(uv_scaled.y - y[1].g) < line) {
        scene_color += y[1] * working_to_display_color;
    }
    if (abs(uv_scaled.y - y[2].b) < line) {
