...
#ifdef IS_IRIS
        if (currentRenderedItemId > 0 && entityId != 10103) {
            material_mask = uint(currentRenderedItemId - 10000);
        } else
#endif

            if (entityId > 0)
            material_mask = uint(entityId - 10000);
        else if (blockEntityId > 0) {
            material_mask = uint(blockEntityId - 10000);
        } else {
            material_mask = 181u; // Transparent
        }

        if (100u <= material_mask && material_mask < 164u) {
            if (material_mask == 102u) {
                material_mask = 80u; // Lightning
            } else if (material_mask == 103u) {
                material_mask = 181u; // Player
            } else if (material_mask == 104u) {
                material_mask = 48u; // Drowned
            }
#ifdef WORLD_END
            else if (material_mask == 105u) {
                material_mask = 62u; // Dragon breath
            }
#endif
        }
        // else if (264u <= material_mask && material_mask < 280u) material_mask
        // += 32u; // Candle Items
...
#if defined(WATER_CAUSTICS) && defined(PHYSICS_MOD_OCEAN) \
    && defined(PHYSICS_OCEAN)
    if (physics_iterationsNormal >= 1.0 && material_mask == 1) {
        // basic texture to determine how shallow/far away from the shore the
        // water is
        physics_localWaviness
            = texelFetch(
                  physics_waviness,
                  ivec2(gl_Vertex.xz) - physics_textureOffset,
                  0
            )
                  .r;
        // transform gl_Vertex (since it is the raw mesh, i.e. not transformed
        // yet)
