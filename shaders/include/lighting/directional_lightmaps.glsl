...
vec3 get_directional_lpv(vec3 normal, vec3 scene_pos, vec3 lpv_blocklight) {
    ...
    blocklight_gradient
        = transpose(mat2x3(dFdx(lpv_blocklight), dFdy(lpv_blocklight)));
    ...
    // blocklight_dir = mat3(normalize(blocklight_dir[0]),
    // normalize(blocklight_dir[1]), normalize(blocklight_dir[2]));
    // blocklight_dir /= determinant(blocklight_dir);

    if (max_of(vec3(
            length_squared(blocklight_gradient[0]),
            length_squared(blocklight_gradient[1]),
            length_squared(blocklight_gradient[2])
        ))
        > 1e-12) {
        blocklight_mul = (clamp01(normalize(normal * blocklight_dir) + 0.8)
                              * DIRECTIONAL_LIGHTMAPS_INTENSITY
                          + (1.0 - DIRECTIONAL_LIGHTMAPS_INTENSITY))
