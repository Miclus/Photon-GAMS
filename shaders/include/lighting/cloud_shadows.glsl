...
float get_cloud_shadows(sampler2D cloud_shadow_map, vec3 scene_pos) {
    ...
    float altitude_fraction = (scene_pos.y + eyeAltitude - SEA_LEVEL)
            * (CLOUDS_SCALE / CLOUDS_CUMULUS_THICKNESS)
        - CLOUDS_CUMULUS_ALTITUDE;
...
vec2 render_cloud_shadow_map(vec2 uv) {
    ...
    t = intersect_sphere(
            ...
            clouds_towering_cumulus_radius
                + 0.25 * clouds_towering_cumulus_thickness
    )
            .y;
    pos = ray_origin + light_dir * t;
    distance_fade = exp2(-distance_fade_strength * length(pos.xy));
    density = clouds_towering_cumulus_density(pos);
    float towering_transmittance = exp(
        -1.00 * distance_fade * extinction_coeff
        * clouds_towering_cumulus_thickness * rcp(abs(light_dir.y) + eps)
        * density
    );
    ...
    vec2 thunderhead_intersect = intersect_spherical_shell(
        ray_origin,
        light_dir,
    ...
    if (thunderhead_intersect.y > 0.0) {
        float ray_length
            = thunderhead_intersect.y - max(0.0, thunderhead_intersect.x);
        const uint shadow_steps = 28u;
        float step_length = ray_length / float(shadow_steps);
        vec3 ray_step = light_dir * step_length;
        vec3 march_pos
            = ray_origin + light_dir * max(0.0, thunderhead_intersect.x);

        float thunderhead_optical_depth = 0.0;
        extinction_coeff = mix(0.1, 0.1, smoothstep(0.0, 0.3, abs(sun_dir.y)))
        ...
        for (uint i = 0u; i < shadow_steps; ++i) {
            ...
            distance_fade
                = exp2(-distance_fade_strength * length(sample_pos.xy));
            density = clouds_thunderhead_density(sample_pos);
            thunderhead_optical_depth
                += density * extinction_coeff * step_length * distance_fade;
        }
        float thunderhead_transmittance
            = exp(-thunderhead_optical_depth * rcp(abs(light_dir.y) + eps));
    ...
    extinction_coeff = mix(0.001, 0.1, day_factor) * CLOUDS_ALTOCUMULUS_DENSITY
        * (0.25 - 0.33 * rainStrength);
    t = intersect_sphere(
            ray_origin,
            light_dir,
