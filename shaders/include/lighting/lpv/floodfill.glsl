...
vec3 tint_nether_lpv_lava_emit(vec3 emitted) {
    const float blue_lava_blend = 0.85;
    float w = clamp(nether_lava_blue_biome * blue_lava_blend, 0.0, 1.0);
    if (w < 1e-6) {
        return emitted;
    }
    float lum = max(dot(emitted, vec3(0.25, 0.58, 0.17)), 1e-5);
    ...
    return mix(emitted, cool_scaled, w);
}
#endif

bool is_emitter(uint block_id) {
    return 32u <= block_id && block_id < 64u
        || block_id > 184u && block_id < 192u
        || block_id > 191u && block_id < 240u || block_id == 182u;
}
...
bool is_custom(uint block_id) { return 64u <= block_id && block_id < 96u; }

bool is_candle(uint block_id) { return 264u <= block_id && block_id < 332u; }
...
vec3 get_emitted_light(uint block_id) {
    if (is_emitter(block_id)) {
        if (32u <= block_id && block_id < 64u) {
...
#if defined WORLD_NETHER && defined NETHER_BIOME_BLUE_LAVA
            if (block_id == 39u) {
                em = tint_nether_lpv_lava_emit(em);
            }
#endif
            return em;
            ...
            if (block_id == 199) {
#if defined WORLD_NETHER && defined NETHER_BIOME_BLUE_LAVA
                return tint_nether_lpv_lava_emit(light_color[39u] * 3);
#else
            ...
            if (block_id == 215) {
#if defined WORLD_NETHER && defined NETHER_BIOME_BLUE_LAVA
                return tint_nether_lpv_lava_emit(light_color[39u] * 6);
#else
            ...
            if (block_id == 231) {
#if defined WORLD_NETHER && defined NETHER_BIOME_BLUE_LAVA
                return tint_nether_lpv_lava_emit(light_color[39u] * 9);
#else
        ...
        if (block_id > 327) { // Uncolored Candle
            return light_color[4u] / 8.0
                * get_candle_intensity(block_id - 328u);
        }
        block_id -= 264u;
...
vec3 get_tint(uint block_id, bool is_transparent) {
    if (is_translucent(block_id)) {
        return texelFetch(light_data_sampler, ivec2(int(block_id) - 164, 1), 0)
            .rgb;
    } else {
...
void update_lpv(writeonly image3D light_img, sampler3D light_sampler) {
    ...
    bool transparent = block_id == 0u || block_id >= 1024u || block_id == 50u
        || block_id == 44u || block_id == 45u || block_id == 46u
        || block_id == 49u || block_id == 36u;
    ...
    emitted_light = sqr(pow(emitted_light, vec3(COLOREDLIGHT_SATURATION)))
        * (1, 3 + (1 - pow(COLOREDLIGHT_SATURATION, 3.0)))
        * sign(emitted_light);
