...
#if ( \
    !defined(WORLD_END) && defined(END_COLORED_LIGHTING) \
    || !defined(END_COLORED_LIGHTING) \
)
const vec3[64] light_color = vec3[64](
    vec3(1.00, 1.00, 1.00) * vec3(1.00, 1.00, 1.00)
        * 12.0, // Strong white light
    vec3(1.00, 1.00, 1.00) * vec3(1.00, 1.00, 1.00) * 6.0, // Medium white light
    vec3(1.00, 1.00, 1.00) * vec3(1.0, 1.0, 1.0) * 1.0, // Weak white light
    vec3(1.00, 0.65, 0.25) * vec3(1.00, 0.65, 0.25)
        * 12.0, // Strong golden light
    vec3(MEDIUM_GOLDEN_LIGHT_R, MEDIUM_GOLDEN_LIGHT_G, MEDIUM_GOLDEN_LIGHT_B)
        * vec3(
            MEDIUM_GOLDEN_LIGHT_R,
            MEDIUM_GOLDEN_LIGHT_G,
            MEDIUM_GOLDEN_LIGHT_B
        )
        * MEDIUM_GOLDEN_LIGHT_I, // Medium golden light
    vec3(1.00, 0.57, 0.30) * vec3(1.00, 0.57, 0.30) * 2.2, // Weak golden light
    vec3(1.00, 0.18, 0.10) * vec3(1.00, 0.18, 0.10)
        * 4.0, // Redstone components
    vec3(LAVA_LIGHT_R, LAVA_LIGHT_G, LAVA_LIGHT_B)
        * vec3(LAVA_LIGHT_R, LAVA_LIGHT_G, LAVA_LIGHT_B) * LAVA_LIGHT_I, // Lava
    vec3(MEDIUM_ORANGE_LIGHT_R, MEDIUM_ORANGE_LIGHT_G, MEDIUM_ORANGE_LIGHT_B)
        * vec3(
            MEDIUM_ORANGE_LIGHT_R,
            MEDIUM_ORANGE_LIGHT_G,
            MEDIUM_ORANGE_LIGHT_B
        )
        * MEDIUM_ORANGE_LIGHT_I, // Medium orange light
    vec3(1.00, 0.63, 0.15) * vec3(1.00, 0.63, 0.15) * 4.0, // Brewing stand
    vec3(1.00, 0.57, 0.30) * vec3(1.00, 0.57, 0.30)
        * 12.0, // Medium golden light (Jack o' Lantern)
    ...
    vec3(0.75, 0.44, 1.00) * vec3(0.75, 0.44, 1.00)
        * 8.0, // Pearlescent froglight
    vec3(0.10, 0.86, 1.0) * vec3(0.10, 0.86, 1.0) * 2.0, // Enchanting table
    vec3(0.75, 0.44, 1.00) * vec3(0.75, 0.44, 1.00) * 4.0, // Amethyst cluster
    vec3(0.75, 0.44, 1.00) * vec3(0.75, 0.44, 1.00)
        * 4.0, // Calibrated sculk sensor
    vec3(0.75, 1.00, 0.83) * vec3(0.75, 1.00, 0.83)
        * 6.0, // Active sculk sensor
...
const vec3[64] light_color = vec3[64](
    ...
    vec3(1.00, 0.65, 0.25) * vec3(1.00, 0.65, 0.25)
        * 12.0, // Strong golden light
    vec3(0.60, 0.10, 1.00) * vec3(0.60, 0.10, 1.00)
        * MEDIUM_GOLDEN_LIGHT_I, // Medium golden light
    vec3(1.00, 0.57, 0.30) * vec3(1.00, 0.57, 0.30) * 2.2, // Weak golden light
    vec3(1.00, 0.18, 0.10) * vec3(1.00, 0.18, 0.10)
        * 4.0, // Redstone components
