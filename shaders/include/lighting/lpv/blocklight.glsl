...
// nether_lava_blue_biome uniform is declared in material.glsl (included above).
// Shift lava-colored LPV energy toward cyan-blue so LPV fog matches surface lava.
vec3 lpv_linear_tint_nether_blue_lava(vec3 lpv_linear) {
    ...
    float lum = max(dot(lpv_linear, vec3(0.25, 0.58, 0.17)), 1e-5);
    vec3 n = lpv_linear / lum;
    float lava_like = smoothstep(0.22, 0.48, n.r - max(n.g, n.b))
        * smoothstep(0.05, 0.20, n.g - n.b)
        * step(n.b + 1e-4, n.r * 0.98);

    const vec3 cool = vec3(0.20, 0.75, 1.55);
    float cool_lum = max(dot(cool, vec3(0.25, 0.58, 0.17)), 1e-5);
    vec3 cool_scaled = cool * (lum / cool_lum);

    return mix(lpv_linear, cool_scaled, w * lava_like);
}
#endif
...
vec3 get_lpv_fog(vec3 scene_pos) {
    return lpv_fog_curve(get_lpv_basic(scene_pos));
}
...
vec3 get_lpv_direction(vec3 scene_pos) {
    ...
    vec3 light_dir = ((lpv_light2.r + lpv_light2.g + lpv_light2.b)
                      - (lpv_light1.r + lpv_light1.g + lpv_light1.b))
        / (epsilon * 2.0);
...
mat3 get_lpv_gradient_rgb(
    vec3 scene_pos,
    float epsilon /*, vec3 normal, vec3 lpv_blocklight*/
) {
    ...
    vec3 dir = ((lpv_light2.r + lpv_light2.g + lpv_light2.b) - (lpv_light1.r +
    lpv_light1.g + lpv_light1.b)) / vec3(epsilon * 2.0); mat3 gradient =
    mat3(dir, dir, dir);*/

    // light_dir = vec3(length(gradient[0]))

    // light_dir = vec3(average3(gradient[0]), average3(gradient[1]),
    // average3(gradient[2]));
...
vec3 get_directional_lpv_mult(
    ...
    mat3 blocklight_dir = transpose(
        get_lpv_gradient_rgb(scene_pos, 0.25 /*, normal, lpv_blocklight*/)
    );

    // if(length_squared(blocklight_dir) > 1e-12) {
    // if (max_of(vec3(length_squared(blocklight_dir[0]),
    // length_squared(blocklight_dir[1]), length_squared(blocklight_dir[2]))) >
    // eps) {
    vec3 NoL = vec3(
        dot(normalize_safe(blocklight_dir[0]), normal),
        dot(normalize_safe(blocklight_dir[1]), normal),
        dot(normalize_safe(blocklight_dir[2]), normal)
    );
    // NoL = vec3(1.0, 0.0, 0.5);
    blocklight_mul = (clamp(NoL, 0.0, 1.0) * DIRECTIONAL_LIGHTMAPS_INTENSITY
                      + (1.0 - DIRECTIONAL_LIGHTMAPS_INTENSITY))
        * inversesqrt(sqrt(lpv_blocklight) + eps);
    //}
...
vec3 get_lpv_direction_fog(vec3 scene_pos) {
    ...
    //vec3 lpv_light1 = get_lpv_basic;clamp01(voxel_pos /
    vec3(voxel_volume_size));
    //vec3 lpv_light2 = get_lpv_basic(scene_pos + epsilon);
    vec3 lpv_light1 = get_lpv_linear(scene_pos - epsilon);
    vec3 lpv_light2 = get_lpv_linear(scene_pos + epsilon);
    vec3 light_dir = ((lpv_light2.x + lpv_light2.y + lpv_light2.z) -
    (lpv_light1.x + lpv_light1.y + lpv_light1.z)) / (epsilon * 2.0);*/
...
vec3 get_lpv_blocklight(
    ...
    if (is_inside_voxel_volume(voxel_pos)) {
        ...
        normal_diff_angle = 1.0
            - clamp01(dot(normalize(normal_diff), normalize(flat_normal)));
        vec3 normal_boost
            = normal_diff + sqrt(normal_diff_angle) * normal_diff * 1.0;
        ...
        vec3 sample_pos
            = clamp01((voxel_pos + lpv_normal) / vec3(voxel_volume_size));
        ...
        /*#if !defined(NO_NORMAL) && defined(DIRECTIONAL_LIGHTMAPS)
                ...
                lpv_blocklight = min(lpv_blocklight, lpv_blocklight2 *
        max_lpv_diff); lpv_blocklight = max(lpv_blocklight, lpv_blocklight2 *
        min_lpv); #endif*/
        ...
        /*#ifdef DIRECTIONAL_LIGHTMAPS
                ...
                //lpv_blocklight *= mix(vec3(1.0), vec3(8.0 *
        normal_diff_angle), ao); lpv_blocklight *= material.f0 * (0.5 +
        normal_diff_angle * 8.0) + (1.0 - material.f0); #endif*/

#if !defined(NO_NORMAL) && defined(DIRECTIONAL_LIGHTMAPS) \
    && DIRECTIONAL_LIGHTMAPS_INTENSITY > 0.0
        // lpv_blocklight *= get_directional_lpv(normal, scene_pos,
        // lpv_blocklight);
        vec3 sample_pos_normal
            = clamp01((voxel_pos + lpv_normal) / vec3(voxel_volume_size));
        vec3 lpv_blocklight_normal = read_lpv_linear(sample_pos_normal);
        lpv_blocklight_normal = sqrt(lpv_blocklight_normal) * ao;
        vec3 lpv_blocklight_directional = lpv_blocklight
            * get_directional_lpv_mult(normal - flat_normal,
                                       scene_pos,
                                       lpv_blocklight);

        vec3 lpv_blocklight_x_directional
            = lpv_blocklight * lpv_blocklight_directional;
        float dot_nd = dot(lpv_blocklight_normal, lpv_blocklight_x_directional);
        if (abs(max_of(lpv_blocklight_normal)
                - max_of(lpv_blocklight * lpv_blocklight_directional))
            < 0.2) {
            lpv_blocklight
                += lpv_blocklight_normal * lpv_blocklight_directional;
        } else {
        ...
        /*#ifdef DIRECTIONAL_LIGHTMAPS
            vec3 world_dir = normalize(scene_pos -
        gbufferModelViewInverse[3].xyz); float light_length =
        length_squared(lpv_blocklight); vec2 light_gradient =
        vec2(dFdx(light_length), dFdy(light_length));
            ...
            if (length_squared(light_gradient) > 1e-12) {
                ...
                vec3 lpv_specular = get_specular_highlight(material, NoL, NoV,
        NoH, LoV, LoH); lpv_blocklight += lpv_specular * lpv_blocklight;
            }
        #endif*/
        ...
        return mix(
            lpv_blocklight * COLORED_LIGHT_I,
            mc_blocklight,
            distance_fade
        );
    } else {
