...
float slice_rel_cdf_cos(float x, float ang_n, float cos_n) {
    ...
    float t0 = n0 * cos_n + n1 * cos(ang_n - 2.0 * phi) + (n2 * ang_n + (n1 * 2.0) * phi + pi) * sin(ang_n);
...
vec4 compute_vbil(
    ...
    for (int s = 0; s < VBIL_SLICES; ++s) {
        ...
        vec2 ray_step_base = (view_to_screen_space(view_pos + view_slice_dir * step_size, true, is_lod) - screen_pos).xy;

        for (float d = -1.0; d <= 1.0; d += 2.0) {
            vec2 ray_step = ray_step_base * d;

            vec2 ray_pos = screen_pos.xy + ray_step * (dither.z + max_of(view_pixel_size) * rcp_length(ray_step));

            for (int i = 0; i < VBIL_STEPS; ++i, ray_pos += ray_step) {
                if (clamp01(ray_pos) != ray_pos) break;

                ivec2 texel = ivec2(ray_pos * view_res * taau_render_scale - 0.5);
                ...
                if (d < 0.0) horizon_angle = horizon_angle.yx;
                if (horizon_angle.x > horizon_angle.y) horizon_angle = horizon_angle.yx;

                horizon_angle.x = slice_rel_cdf_cos(horizon_angle.x, ang_n, cos_n);
                horizon_angle.y = slice_rel_cdf_cos(horizon_angle.y, ang_n, cos_n);

                horizon_angle = clamp01(horizon_angle + dither.w * (1.0 / 32.0));

                uvec2 horizon_int = uvec2(horizon_angle * 32.0);
                uint mask_end = horizon_int.x < 32u ? 0xFFFFFFFFu << horizon_int.x : 0u;
                uint mask_start = horizon_int.y != 0u ? 0xFFFFFFFFu >> (32u - horizon_int.y) : 0u;
                uint step_mask = mask_end & mask_start;

                uint visible_bits = step_mask & (~occlusion_mask);

                if (visible_bits != 0u) {
                    float visibility = float(bitCount(visible_bits)) * (1.0 / 32.0);
