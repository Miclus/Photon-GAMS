...
float ACEScct_to_lin(float acescct) {
    if (acescct > Y_BRK) {
        return exp2(acescct * 17.52 - 9.72);
    } else {
        return (acescct - B) / A;
    }
}
...
vec3 ACEScct_to_ACES(vec3 acescct) {
    vec3 ap1_lin;
    ap1_lin.x = ACEScct_to_lin(acescct.x);
    ap1_lin.y = ACEScct_to_lin(acescct.y);
    ap1_lin.z = ACEScct_to_lin(acescct.z);
...
vec3 gamma_adjust_linear(vec3 rgb_in, float GAMMA, float PIVOT) {
    ...
    vec3 rgb_out = rgb_in;
    if (rgb_in.x > 0.0) {
        rgb_out.x = pow(rgb_in.x, GAMMA) * SCALAR;
    }
    if (rgb_in.y > 0.0) {
        rgb_out.y = pow(rgb_in.y, GAMMA) * SCALAR;
    }
    if (rgb_in.z > 0.0) {
        rgb_out.z = pow(rgb_in.z, GAMMA) * SCALAR;
    }
    return rgb_out;
}
...
vec3 rgb_2_ych(vec3 rgb) { return yab_2_ych(rgb_2_yab(rgb)); }

vec3 ych_2_rgb(vec3 ych) { return yab_2_rgb(ych_2_yab(ych)); }
...
vec3 lmt_pfe(vec3 aces) {
    ...
    if (GAMMA.x != 1.0) {
        aces = gamma_adjust_linear(aces, GAMMA.x, GAMMA.y);
    }
