...
vec3 get_wind_displacement(
    ...
    vec3 wobble = sin(world_pos) + 0.5 * sin(2.0 * world_pos)
        + WAVING_CLEAR_STRENGTH * sin(WAVING_RAIN_STRENGTH * world_pos);
        ...
        case MATERIAL_OPEN_EYEBLOSSOM:
            return world_pos
                + (get_wind_displacement(
                   ...
                   + player_displacement * PLAYER_DISPLACEMENT_PLANTS_STRENGTH)
                * float(is_top_vertex);
        ...
        case MATERIAL_LEAVES:
            return world_pos
                + (get_wind_displacement(
                   ...
                   + player_displacement * PLAYER_DISPLACEMENT_LEAVES_STRENGTH);
#endif
...
// Optifine #if fix, added boolean settings, removed 0.0 as SIZE sliders values
// to avoid comparing float
...
// scene_pos += cameraPosition;
...
#elif CURVATURE_MODE == CURVATURE_MODE_BASIC_SPHERICAL \
    || CURVATURE_MODE == CURVATURE_MODE_LOG_SPHERICAL
    ...
    if (phi > pi) {
        return vec3(
            0,
            intBitsToFloat(0xff800000),
            0
        ); // negative infinity ¯\_(o_o)_/¯
    }

    float h_sin_phi = h * sin(phi);
    scene_pos.y
        = h * cos(phi) /*+ 128.0*/ /* exp(128.0 / CURVATURE_SIZE)*/ - size;
