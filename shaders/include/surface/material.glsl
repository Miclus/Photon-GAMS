...
const Material water_material = Material(
    ...
    false,
    false
); // Prevent specular blowout caused by low skylight intensity
...
void decode_specular_map(vec4 specular_map, inout Material material) {
    ...
    if (specular_map.g < 229.5 / 255.0) {
        ...
        material.f0 = max(material.f0, specular_map.g)
            * SKYLIGHT_I; // Prevent specular blowout caused by low skylight
                          // intensity
...
#ifdef NETHER_LAVA_CRUST
#if defined WORLD_NETHER

float nether_lava_hash(vec2 p) {
    return fract(sin(dot(p, vec2(127.1, 311.7))) * 43758.5453);
}

float nether_lava_value_noise(vec2 p) {
    vec2 i = floor(p);
    vec2 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
    float a = nether_lava_hash(i);
    float b = nether_lava_hash(i + vec2(1.0, 0.0));
    float c = nether_lava_hash(i + vec2(0.0, 1.0));
    float d = nether_lava_hash(i + vec2(1.0, 1.0));
    return mix(mix(a, b, f.x), mix(c, d, f.x), f.y);
}

float nether_lava_fbm(vec2 p) {
    float v = 0.0;
    float a = 0.5;
    mat2 rot = mat2(0.8, 0.6, -0.6, 0.8);
    for (int i = 0; i < 4; ++i) {
        v += a * nether_lava_value_noise(p);
        p = rot * p * 2.08 + vec2(17.7);
        a *= 0.5;
    }
    return clamp01(v);
}
...
#if defined WORLD_NETHER && defined NETHER_BIOME_BLUE_LAVA
uniform float nether_lava_blue_biome;
#endif
...
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 0.45 * linear_step(0.13, 0.4, hsl.z);
                            material.roughness = sqr(1.0 - smoothness);
                            material.f0 = material.albedo;
...
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 1.13 * sin(linear_step(0.0, 1.0, hsl.z));
                            material.roughness
                                = min(sin(1.0 - smoothness), 0.65);
...
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 1.1 * sin(sqrt(linear_step(0.1, 0.9, hsl.z)));
                            material.roughness
                                = min(sin(sqrt(1.0 - smoothness)), 0.7);
                            material.f0
                                = sqr(material.albedo) * 1.8 + vec3(0.01);
                            material.is_metal = true;
                            material.ssr_multiplier = 1.0;
#endif
                        }
...
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 0.45 * smoothstep(0.01, 0.7, hsl.z);
                            material.roughness = sqr(1.0 - smoothness);
                            material.f0 = vec3(0.02);
#endif
                        } else { // 17
...
// Particles
                            ...
                            material.emission = PARTICLES_BRIGHTNESS
                                * (0.47 * material.albedo
                                       * step(0.378, redness * l)
                                   + 2.5 * albedo_sqrt
                                       * (0.2
                                          + 0.8 * isolate_hue(hsl, 15.0, 15.0))
                                       * step(0.871, hsl.y)
                                       * step(0.35, hsl.z));

#if defined WORLD_END && defined END_COLORED_LIGHTING
                            material.emission -= 0.2 * albedo_sqrt
                                * linear_step(0.78, 0.85, hsl.z);
                            material.emission += 0.3
                                * sqr(sqr(vec3(0.70, 0.10, 1.0) * 2))
                                * linear_step(0.78, 0.85, hsl.z)
                                * float(albedo_sqrt);
#endif
                    ...
                    if (material_mask < 30) { // 28-30
                        if (material_mask == 28u) { // 28  Ore
...
#endif
#ifdef HARDCODED_ORE
                            material.emission = 0.07 * ORE_BRIGHTNESS
                                * albedo_sqrt * albedo_sqrt
                                * (0.9 + 0.1 * isolate_hue(hsl, 15.0, 0.1))
                                * step(0.01, hsl.y) * step(0.00001, hsl.x)
                                * step(0.35, hsl.z);
#endif
                            ...
                            float l = 0.5 * (min_of(ap1) + max_of(ap1));
                            float redness = ap1.r * rcp(ap1.g + ap1.b);
                            material.emission = 0.5 * ORE_BRIGHTNESS
                                    * material.albedo * step(0.40, redness * l)
                                + 0.3 * albedo_sqrt * albedo_sqrt
                                    * (0.2 + 0.8 * isolate_hue(hsl, 15.0, 15.0))
                                    * step(0.10, hsl.y) * step(0.76, hsl.z);
#endif
                        }
                    } else { // 30-32
...
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 0.45 * linear_step(0.4, 0.8, hsl.z);
                            material.roughness = sqr(1.0 - smoothness);
                            material.f0 = vec3(0.02);
#endif
                ...
                if (material_mask < 36u) { // 32-36
...
#ifdef HARDCODED_EMISSION
                            // Weak white light
                            material.emission = vec3(1.0) * albedo_sqrt * 0.45
                                - 0.449 * albedo_sqrt * step(0.20, hsl.x)
                                    * step(0.10, hsl.y) * step(0.15, hsl.z);
#endif
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 0.45 * smoothstep(0.01, 0.7, hsl.z);
                    ...
                    if (material_mask < 38u) { // 36-38
                        if (material_mask == 36u) { // 36
...
#if defined WORLD_END && defined END_COLORED_LIGHTING
                            material.emission = 0.6
                                * sqr(vec3(0.70, 0.10, 1.0) * 3)
                                * linear_step(0.78, 0.85, hsl.z);
#endif
                            light_levels.x *= 0.85;
#endif
                            ...
                            material.emission = 1.4 * albedo_sqrt
                                * linear_step(0.47,
                                              0.50,
                                              0.2 * hsl.y + 0.49 * hsl.z
                                                  + 0.1 * blue);
#endif
#ifdef HARDCODED_SPECULAR
                            float smoothness
...
#ifdef HARDCODED_EMISSION
                            // Lava
                            material.emission = LAVA_EMISSION_I * 4.0
                                * albedo_sqrt
                                * (0.2 + 0.8 * isolate_hue(hsl, 30.0, 15.0))
                                * step(0.4, hsl.y);

#if defined LAVA_EMISSION_RGB && !defined RENDERTYPE_ENTITY
                            material.albedo = float(material.albedo)
                                * vec3(LAVA_EMISSION_R,
                                       LAVA_EMISSION_G,
                                       LAVA_EMISSION_B);
#endif
...
#if defined WORLD_NETHER && defined NETHER_BIOME_BLUE_LAVA
                            {
                                const float blue_lava_blend = 0.85;
                                float w = clamp01(nether_lava_blue_biome * blue_lava_blend);
                                if (w > eps) {
                                    float em_lum = max(
                                        dot(
                                            material.emission,
                                    ...
                                    vec3 cool_em = vec3(0.20, 0.75, 1.55) * em_lum;
                                    material.emission = mix(material.emission, cool_em, w);
                                    vec3 cool_alb = material.albedo
                                        * vec3(0, 40, 255);
                                    material.albedo = mix(
                                        material.albedo,
                                        cool_alb,
...
#if defined WORLD_END && defined END_COLORED_LIGHTING
                            material.emission = -1.0 * albedo_sqrt
                                * (0.2 + 0.8 * isolate_hue(hsl, 30.0, 15.0))
                                * step(0.4, hsl.y);
                            material.emission += 2.0 * float(material.albedo)
                                * (0.2 + 0.8 * isolate_hue(hsl, 30.0, 15.0))
                                * step(0.4, hsl.y) * 0.6
                                * sqr(vec3(0.70, 0.10, 1.0) * 3);
#endif
...
#if defined WORLD_NETHER
                            {
                                vec2 p = world_pos.xz * NETHER_LAVA_CRUST_SCALE;
#ifdef NETHER_LAVA_CRUST_ANIMATION
                                float anim_t = frameTimeCounter
                                    * NETHER_LAVA_CRUST_ANIM_SPEED
                                    * (0.034
                                       / max(NETHER_LAVA_CRUST_SCALE, 0.05));
                                vec2 flow = vec2(0.41, -0.29) * anim_t;
                                vec2 swirl = vec2(
                                                     cos(anim_t * 0.15),
                                                     sin(anim_t * 0.15)
                                                 )
                                ...
                                p += flow + swirl;
#endif
                                float n = nether_lava_fbm(p);
                                float crust
                                    = smoothstep(0.1, 0.84, n);
                                float w = crust * 1.1;
                                material.emission *= mix(vec3(1.0), vec3(0.1), w);
                                vec3 crust_albedo
                                    = vec3(0.025, 0.012, 0.008);
                                material.albedo
                                    = mix(material.albedo, crust_albedo, w * 0.88);
                            }
#endif
#endif
...
#if defined WORLD_END && defined END_COLORED_LIGHTING
                            material.emission -= 1.5 * albedo_sqrt
                                * (0.1 + 0.9 * cube(hsl.z));
                            material.emission += 0.12
                                * sqr(sqr(vec3(0.70, 0.10, 1.0) * 3))
                                * linear_step(0.1, 0.9, hsl.z)
                                * float(albedo_sqrt);
#endif
#endif
#ifdef HARDCODED_SPECULAR
...
#ifdef HARDCODED_EMISSION
                            // End portal frame
                            material.emission = 0.45 * albedo_sqrt
                                    * isolate_hue(hsl, 120.0, 50.0)
                                + 0.45 * albedo_sqrt
                                    * isolate_hue(hsl, 50.0, 50.0)
                                    * step(0.8, hsl.y);
#endif
#ifdef HARDCODED_SPECULAR
                            float smoothness
...
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 0.5 * linear_step(0.03, 0.01, hsl.z);
                            material.roughness = sqr(1.0 - smoothness);
                            material.f0 = vec3(0.02);
                ...
                if (material_mask < 52u) { // 48-52
                        ...
                        if (material_mask == 50u) { // 50
                            ...
                            material.emission
                                = (0.2 * material.albedo
                                   ...
                                   + 0.3
                                       * (0.38
                                          + 0.02 * isolate_hue(hsl, 15.0, 15.0))
                                       * step(0.771, hsl.y) * step(0.0, hsl.z));
#endif
#ifdef HARDCODED_SSS
...
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 0.75 * sin(smoothstep(0.2, 1.0, hsl.z));
                            material.roughness
                                = min(sin(1.1 - smoothness), 0.95);
                            material.f0
                                = material.albedo * vec3(0.168, 0.043, 0.043);
#endif
                        } else { // 51
#ifdef HARDCODED_EMISSION
                            // Ochre froglight
                    ...
                    if (material_mask < 54u) { // 52-54
                        ...
                        if (material_mask == 54u) { // 54
                            ...
                            material.emission = 0.66 * albedo_sqrt
                                * linear_step(0.97, 1.0, blue + hsl.z);
#endif
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 0.5 * linear_step(0.03, 0.01, hsl.z);
...
#ifdef HARDCODED_EMISSION
                            // Amethyst cluster
                            material.emission = vec3(0.35) * 2.0 * albedo_srgb
                                * sin((0.1 + 0.9 * hsl.z)) * albedo_sqrt
                                * pow4(hsl.z);
#endif
#ifdef HARDCODED_SPECULAR
                            float smoothness
                                = 0.4 * linear_step(0.1, 0.2, hsl.z);
                            material.roughness = sqr(1.0 - smoothness);
                            material.f0 = vec3(0.02);
                            material.ssr_multiplier = 1.0;
#endif
...
// Purple weak light
#ifdef HARDCODED_EMISSION
                            material.emission = 0.80 * albedo_sqrt * albedo_srgb
                                * step(0.8,
                                       0.1 * hsl.y + 0.7 * hsl.z
                                           + 0.19 * hsl.x);
#endif
    ...
    // 			if (material_mask > 191u && material_mask < 240u ||
    // material_mask > 63u && material_mask < 80u) { // 264 - 331
    // 	// 343
    // 	#ifdef HARDCODED_EMISSION
    ...
    if (material_mask == 192u || material_mask == 208u || material_mask == 64u
        || material_mask == 224u) { // 264 - 331
// Red
#ifdef HARDCODED_EMISSION
    ...
    if (material_mask == 193u || material_mask == 209u || material_mask == 65u
        || material_mask == 225u) { // 264 - 331
// Orange
#ifdef HARDCODED_EMISSION
        material.emission = material.albedo * isolate_hue(hsl, 15.0, 50)
            * linear_step(0.2, 1.0, hsl.z);
#endif
#ifdef HARDCODED_SPECULAR
        float smoothness = 0.45 * smoothstep(0.01, 0.7, hsl.z);
        material.roughness = sqr(1.0 - smoothness);
    ...
    if (material_mask == 194u || material_mask == 210u || material_mask == 66u
        || material_mask == 226u) { // 264 - 331
// Yellow
#ifdef HARDCODED_EMISSION
        material.emission = max(
            material.albedo * isolate_hue(hsl, 45.0, 20),
            material.albedo * linear_step(0.7, 0.8, hsl.z)
        );
#endif
#ifdef HARDCODED_SPECULAR
        float smoothness = 0.45 * smoothstep(0.01, 0.7, hsl.z);
    ...
    if (material_mask == 195u || material_mask == 211u || material_mask == 67u
        || material_mask == 227u) { // 264 - 331
// Brown
#ifdef HARDCODED_EMISSION
        material.emission = max(
            material.albedo * isolate_hue(hsl, 45.0, 25),
            material.albedo * linear_step(0.7, 0.8, hsl.z)
        );
#endif
#ifdef HARDCODED_SPECULAR
    ...
    if (material_mask == 196u || material_mask == 212u || material_mask == 68u
        || material_mask == 228u || material_mask == 197u
        || material_mask == 213u || material_mask == 69u
        || material_mask == 229u) { // 264 - 331
// Green, Lime
#ifdef HARDCODED_EMISSION
        material.emission = max(
            material.albedo * isolate_hue(hsl, 105.0, 40),
            material.albedo * linear_step(0.5, 0.8, hsl.z)
        );
#endif
#ifdef HARDCODED_SPECULAR
        float smoothness = 0.45 * smoothstep(0.01, 0.7, hsl.z);
    ...
    if (material_mask > 197u && material_mask < 201u
        || material_mask > 213u && material_mask < 217u
        || material_mask > 69u && material_mask < 73u
        || material_mask > 229u && material_mask < 233u) { // 264 - 331
// Blue, Light blue, Cyan
#ifdef HARDCODED_EMISSION
        material.emission = max(
            material.albedo * isolate_hue(hsl, 230.0, 70),
            material.albedo * linear_step(0.7, 0.8, hsl.z)
        );
#endif
#ifdef HARDCODED_SPECULAR
    ...
    if (material_mask > 200u && material_mask < 203u
        || material_mask > 216u && material_mask < 219u
        || material_mask > 72u && material_mask < 75u
        || material_mask > 232u && material_mask < 235u) { // 264 - 331
// Purple, Magenta
#ifdef HARDCODED_EMISSION
        material.emission
            = material.albedo * albedo_srgb * isolate_hue(hsl, 300, 42);
#endif
#ifdef HARDCODED_SPECULAR
        float smoothness = 0.45 * smoothstep(0.01, 0.7, hsl.z);
        material.roughness = sqr(1.0 - smoothness);
    ...
    if (material_mask == 203u || material_mask == 219u || material_mask == 75u
        || material_mask == 235u) { // 264 - 331
// Pink
#ifdef HARDCODED_EMISSION
        material.emission
            = material.albedo * albedo_srgb * isolate_hue(hsl, 300, 50);
#endif
#ifdef HARDCODED_SPECULAR
        float smoothness = 0.45 * smoothstep(0.01, 0.7, hsl.z);
        material.roughness = sqr(1.0 - smoothness);
    ...
    if (material_mask > 203u && material_mask < 208u
        || material_mask > 219u && material_mask < 224u
        || material_mask > 75u && material_mask < 80u
        || material_mask > 235u && material_mask < 240u) { // 264 - 331
// Black, White, Gray, Light gray
#ifdef HARDCODED_EMISSION
        material.emission = max(
            material.albedo * isolate_hue(hsl, 50.0, 40),
            material.albedo * linear_step(0.7, 0.8, hsl.z)
        );
#endif
#ifdef HARDCODED_SPECULAR
...
// Obsidian
    ...
    if (material_mask > 184u && material_mask < 190u) { // Ore
        ...
        float l = 0.5 * (min_of(ap1) + max_of(ap1));
        float redness = ap1.r * rcp(ap1.g + ap1.b);
        material.emission = 0.2 * ORE_BRIGHTNESS * albedo_sqrt
            * step(0.0,
                   normalize(
                       step(0.40, redness * l)
                       + 0.3 * albedo_sqrt * albedo_sqrt
                           * (0.2 + 0.8 * isolate_hue(hsl, 180.0, 180.0))
                           * step(0.10, hsl.y) * step(0.4, hsl.z)
                   ));
#endif
    }
        ...
        material.emission = 0.2 * ORE_BRIGHTNESS * albedo_sqrt
            * step(0.0,
                   normalize(
                       ...
                       + 0.3 * albedo_sqrt * albedo_sqrt
                           * (0.2 + 0.8 * isolate_hue(hsl, 180.0, 180.0))
                           * step(0.1, hsl.y) * step(0.76, hsl.z)
                   ));
#endif
    }
...
#ifdef HARDCODED_EMISSION
        material.emission
            = isolate_hue(hsl, 310.0, 50.0) * 2.2 * albedo_sqrt * albedo_sqrt;
#endif
#ifdef HARDCODED_SPECULAR
...
//
#ifdef HARDCODED_EMISSION
        material.emission = 0.50 * albedo_sqrt
            * step(0.8, 0.1 * hsl.y + 0.7 * hsl.z + 0.3 * hsl.x);
#endif
#ifdef HARDCODED_SPECULAR
        float smoothness = 0.4 * smoothstep(0.2, 0.6, hsl.z);
        material.roughness = sqr(1.0 - smoothness);
