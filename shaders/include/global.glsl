...
#if defined(WORLD_MOON) || defined(WORLD_MARS) || defined(WORLD_VENUS) \
    || defined(WORLD_MERCURY)
#define WORLD_SPACE
#endif

#define SKY_MAP_LIGHT_X \
    (int(view_res.x * float(SKY_MAP_RESOLUTION) / 100.0 + 0.5) - 1)
...
uint max_of(uint a, uint b, uint c, uint d) {
...
uint min_of(uint a, uint b, uint c, uint d) {
...
vec3 sdiv(vec3 a, vec3 b) {
    return vec3(sdiv(a.x, b.x), sdiv(a.y, b.y), sdiv(a.z, b.z));
}
...
#define log_polar_to_cartesian(logp) \
    (exp(logp.x) * vec2(cos(logp.y), sin(logp.y)))
