...
vec2 cave_fog_density(vec3 world_pos) {
    ...
    float noise
        = texture(NOISE_3D, 0.015 * world_pos + wind * frameTimeCounter).x;
...
mat2x3 raymarch_cave_fog(
    ...
    float distance_to_upper_plane
        = (cave_fog_volume_top - eyeAltitude) / world_dir.y;
...
#define shadow 1.0
        ...
        // const vec4 lpv_fog_params = vec4(-1.341e-2, 6.223e-1, 3.663e-2,
        // -1.623e-1);

        // lpv_color = pow(lpv_fog_params.x * lpv_color, vec3(3.0)) +
        // sqr(lpv_fog_params.y * lpv_color) + lpv_fog_params.z * lpv_color
        // + 1.0;
#else
#define lpv_color 0.0
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        scattering
            += scatter_amount * (light_lpv * vec2(isotropic_phase, mie_phase));
