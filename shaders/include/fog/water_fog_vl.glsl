...
mat2x3 raymarch_water_fog(
    ...
    const vec3 absorption_coeff_below
        = vec3(
    ...
    const vec3 extinction_coeff_below
        = absorption_coeff_below + scattering_coeff_below;

    const vec3 absorption_coeff_above
        = vec3(WATER_ABSORPTION_R, WATER_ABSORPTION_G, WATER_ABSORPTION_B)
        * rec709_to_working_color;
    const vec3 scattering_coeff_above = vec3(WATER_SCATTERING);
    const vec3 extinction_coeff_above
        = absorption_coeff_above + scattering_coeff_above;

    const uint multiple_scattering_iterations
        = FOG_MULTIPLE_SCATTERING_ITERATIONS; // 4

    vec3 absorption_coeff
        = in_water ? absorption_coeff_below : absorption_coeff_above;
    vec3 scattering_coeff
        = in_water ? scattering_coeff_below : scattering_coeff_above;
    vec3 extinction_coeff
        = in_water ? extinction_coeff_below : extinction_coeff_above;
...
#if defined CLOUD_SHADOWS && defined FOG_CLOUD_SHADOWS \
    && defined WORLD_OVERWORLD
...
#define distance_traveled 0.0
        ...
        for (uint i = 0u; i < multiple_scattering_iterations; ++i) {
            ...
            // Skylight
            scattering += ambient_color * isotropic_phase * sky_transmittance
                * transmittance * scattering_amount;
