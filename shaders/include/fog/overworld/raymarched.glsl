...
vec2 air_fog_density(vec3 world_pos) {
    ...
    /*const vec3 wind = 0.0001 * AIR_FOG_CLOUDY_NOISE_CLOUD_SPEED *
    vec3(AIR_FOG_CLOUDY_NOISE_CLOUD_X, 0.0, AIR_FOG_CLOUDY_NOISE_CLOUD_Z);

    // Controls the spacing between clouds
    // Lower values = more spread out clouds, Higher values = tighter packed
    clouds
    // Default: 0.001
    ...
    float noise = texture(noisetex, cloud_spacing * world_pos.xz + wind.xz *
    frameTimeCounter).w;
...
mat2x3 raymarch_air_fog(
    ...
    const uint air_fog_multiple_scattering_iterations
...
#endif
#elif defined CLOUD_SHADOWS && defined FOG_CLOUD_SHADOWS \
    && defined WORLD_OVERWORLD
...
#define shadow 1.0
        ...
        vec3 lightning_scattering_coeff
            = fog_params.rayleigh_scattering_coeff * density.x
            + fog_params.mie_scattering_coeff * density.y;
        light_lightning += lightning_flash_fog_sample_scattering(
            world_pos - cameraPosition,
            lightning_scattering_coeff,
            visible_scattering
        );
