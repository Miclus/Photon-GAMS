...
// just some basic consts for the wave function based on afl_ext's shader
// https://www.shadertoy.com/view/Xdlczl the overall shape must stay consistent
// because it is also computed on the CPU side to offset entities (though a
// custom CPU integration of your shader is possible by contacting me on my
// discord server https://discord.gg/VsNs9xP)
...
// this is the surface detail from the physics options, ranges from 13 to 48
// (yeah I know weird)
uniform int physics_iterationsNormal;
// used to offset the 0 point of wave meshes to keep the wave function
// consistent even though the mesh totally changes
uniform vec2 physics_waveOffset;
// used for offsetting the local position to fetch the right pixel of the
// waviness texture
uniform ivec2 physics_textureOffset;
// time in seconds that can go faster dependent on weather conditions (affected
// by weather strength multiplier in ocean settings
...
// just the generic minecraft lightmap, you can remove this and use the one
// supplied by Optifine/Iris
...
float physics_waveHeight(
    ...
    position = (position - physics_waveOffset) * PHYSICS_XZ_SCALE
        * physics_oceanWaveHorizontalScale;
    float iter = 0.0;
    ...
    return height / waveSum * physics_oceanHeight * factor
        - physics_oceanHeight * factor * 0.5;
}
...
vec3 physics_waveNormal(
    ...
    vec3 waveNormal = normalize(vec3(
        direction.x * totalFactor,
        PHYSICS_NORMAL_STRENGTH,
        direction.y * totalFactor
    ));

    vec2 eyePosition = position + physics_modelOffset.xz;
    vec2 rippleFetch = (eyePosition + vec2(physics_rippleRange))
        / (physics_rippleRange * 2.0);
...
vec3 physics_waveNormal(
    ...
    const vec2 direction
        = -physics_waveDirection(position.xy, physics_iterationsNormal, time);
    float oceanHeightFactor = physics_oceanHeight / 13.0;
    float totalFactor = oceanHeightFactor * factor;
    vec3 waveNormal = normalize(vec3(
        direction.x * totalFactor,
...
WavePixelData physics_wavePixel(
    ...
    data.worldPos
        = wavePos / physics_oceanWaveHorizontalScale / PHYSICS_XZ_SCALE;
    data.height = height / waveSum * physics_oceanHeight * factor
        - physics_oceanHeight * factor * 0.5;
    ...
    vec2 waterUV = mix(
        position - physics_waveOffset,
        data.worldPos,
        clamp(factor * 2.0, 0.2, 1.0)
    );

    vec2 s1
        = textureLod(physics_foam, vec3(waterUV * 0.26, time / 360.0), 0).rg;
    vec2 s2
        = textureLod(physics_foam, vec3(waterUV * 0.02, time / 360.0 + 0.5), 0)
              .rg;
    vec2 s3
        = textureLod(physics_foam, vec3(waterUV * 0.1, time / 360.0 + 1.0), 0)
              .rg;

    float waterSurfaceNoise = s1.r * s2.r * s3.r * 2.8 * physics_foamAmount;
    waveAmplitude = clamp(waveAmplitude * 1.2, 0.0, 1.0);
    float blend = cube(factor);
    float blendNear = sqrt(factor);
    // waterSurfaceNoise = mix(waterSurfaceNoise * waveAmplitude, (1.0 -
    // waveAmplitude) * waterSurfaceNoise + waveAmplitude * physics_foamAmount,
    // blend);
    float noiseNear
        = waterSurfaceNoise * blendNear + waterSurfaceNoise * waveAmplitude;
    // mix(waterSurfaceNoise * sqrt(factor), waterSurfaceNoise, waveAmplitude);
    float noiseFar = (1.0 - waveAmplitude) * waterSurfaceNoise
        + waveAmplitude * physics_foamAmount;
    waterSurfaceNoise = mix(noiseNear, noiseFar, blend);
    // waterSurfaceNoise = mix(waterSurfaceNoise * sqrt(factor) +
    // waterSurfaceNoise * waveAmplitude, (1.0 - waveAmplitude) *
    // waterSurfaceNoise + waveAmplitude * physics_foamAmount, blend);
    // waterSurfaceNoise = (1.0 - waveAmplitude) * waterSurfaceNoise * blend +
    // waveAmplitude * mix(waterSurfaceNoise, physics_foamAmount, blend);

    float worleyNoise = 0.2 + 0.8 * s1.g * (1.0 - s2.g);
    float waterFoamMinSmooth = 0.45;
    float waterFoamMaxSmooth = 2.0;
    waterSurfaceNoise
        = smoothstep(waterFoamMinSmooth, 1.0, waterSurfaceNoise) * worleyNoise;

    data.foam
        = clamp01(waterFoamMaxSmooth * waterSurfaceNoise * physics_foamOpacity);

    return data;
}
#endif // PHYSICS_MOD_OCEAN

/*// VERTEX STAGE
void main() {
    // basic texture to determine how shallow/far away from the shore the water
is physics_localWaviness = texelFetch(physics_waviness, ivec2(gl_Vertex.xz) -
physics_textureOffset, 0).r;
    // transform gl_Vertex (since it is the raw mesh, i.e. not transformed yet)
    vec4 finalPosition = vec4(gl_Vertex.x, gl_Vertex.y +
physics_waveHeight(gl_Vertex.xz, PHYSICS_ITERATIONS_OFFSET,
physics_localWaviness, physics_gameTime), gl_Vertex.z, gl_Vertex.w);
    // pass this to the fragment shader to fetch the texture there for per
fragment normals physics_localPosition = finalPosition.xyz;
...
// FRAGMENT STAGE
void main() {
    WavePixelData wave = physics_wavePixel(physics_localPosition.xz,
physics_localWaviness, physics_iterationsNormal, physics_gameTime);

    // access the wave struct data however you want, wave.normal is in world
space, wave.foam is the final foam amount
}*/
