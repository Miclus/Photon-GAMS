...
vec3 draw_stars(vec3 ray_dir, float galaxy_luminance) {
    ...
    float star_threshold = 1.0
        - 0.008 * STARS_COVERAGE * smoothstep(-0.2, 0.5, -sun_dir.y)
...
const float sun_luminance
    = SUN_LUMINANCE * SUN_DISK_INTENSITY; // luminance of sun disk
const float moon_luminance
    = MOON_LUMINANCE * MOON_DISK_INTENSITY; // luminance of moon disk
...
vec3 draw_galaxy(vec3 ray_dir, out float galaxy_luminance) {
    ...
    const vec3 galaxy_tint
        = vec3(GALAXY_TINT_R, GALAXY_TINT_G, GALAXY_TINT_B) * GALAXY_INTENSITY;
    // Check if it's night time
    // if (sun_dir.y > -0.05) return vec3(0.0); // Return black if it's not
    // night
...
vec3 draw_galaxy(vec3 ray_dir, out float galaxy_luminance) {
    ...
    vec3 galaxy
        = texture(galaxy_sampler, vec2(lon * rcp(tau) + 0.1, lat * rcp(pi)))
              .rgb;
...
vec3 adjust_night_atmosphere(vec3 atmosphere, vec3 ray_dir) {
    ...
    vec3 blue_hour_sky
        = mix(atmosphere, atmosphere * blue_hour_tint, blue_hour);
...
vec3 draw_sky(
    ...
    // Shader moon (see draw_moon); gbuffers moon quad writes black to avoid UV
    // issues
    ...
    vec3 rain_sky
        = get_weather_color() * (1.0 - exp2(-0.8 / clamp01(ray_dir.y)));
    sky = mix(
        sky,
        rain_sky,
...
vec3 draw_sky(vec3 ray_dir) {
    // Ungraded scattering for cloud fill / aerial perspective; grade only the
    // sky background
    ...
    vec3 atmosphere_display = atmosphere_sky_color_grade(atmosphere);
    return draw_sky(
        ray_dir,
        atmosphere_display,
        clouds_and_aurora,
...
const vec3 end_sun_color = vec3(
    END_SOLAR_FLARE_COLOR_R,
    END_SOLAR_FLARE_COLOR_G,
    END_SOLAR_FLARE_COLOR_B
);
...
vec3 draw_sun(vec3 ray_dir) {
    ...
    vec3 tangent = sun_dir.y == 11.0
        ? vec3(1.0, 0.0, 0.0)
        : normalize(cross(vec3(1.0, 0.0, 0.0), sun_dir));
    ...
    float flare = texture(
                      noisetex,
                      vec2(theta, r - END_SOLAR_FLARE_SPEED * frameTimeCounter)
    )
                      .x;
    flare
        = pow5(flare) * exp(END_SOLAR_FLARE_FALLOFF * (r - sun_angular_radius));
    flare = r < sun_angular_radius ? 0.0 : flare;

    return end_sun_color * rcp(sun_solid_angle)
        * max0(sun_disk + END_SOLAR_FLARE_INTENSITY * flare);
#else
...
vec3 draw_sun(vec3 ray_dir) {
    ...
    return sun_luminance * vec3(1.0) * step(0.0, center_to_edge)
        * limb_darkening;
}
