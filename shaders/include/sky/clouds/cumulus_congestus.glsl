...
float clouds_cumulus_congestus_density(vec3 pos) {
    ...
    float density = 1.5
        * sqr(linear_step(
            0.75 - 0.15 * clamp01(clouds_params.cumulus_congestus_blend),
            1.0,
            sqrt(noise)
        ));
    density
...
CloudsResult draw_cumulus_congestus_clouds(
    ...
    const uint primary_steps_const = CLOUDS_CUMULUS_CONGESTUS_PRIMARY_STEPS;
    ...
    if (
        ...
        || planet_intersected
            && length(air_viewer_pos)
                < clouds_cumulus_congestus_radius // planet blocking clouds
    ) {
    ...
    float in_col = smoothstep(
                       clouds_cumulus_congestus_distance,
                       clouds_cumulus_congestus_distance + 2000.0,
                       xz
                   )
        * (1.0
           - smoothstep(
               clouds_cumulus_congestus_end_distance - 2000.0,
               clouds_cumulus_congestus_end_distance,
               xz
           ));
    float immersion
