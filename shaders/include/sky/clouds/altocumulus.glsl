...
// Scales altocumulus shell-occlusion onto cumulus / lower towering samples.
#ifndef PHOTON_ALTOCUMULUS_LOWER_SHADOW_SCALE
#define PHOTON_ALTOCUMULUS_LOWER_SHADOW_SCALE 1.55
#endif
...
// ∫ρ ds along light_dir through the altocumulus shell only. Used when the
// shading sample sits in a lower layer (e.g. towering cumulus): unlike
// clouds_altocumulus_optical_depth, this intersects the shell first so the
// march covers the full light chord through ac instead of short steps that
// never reach the layer from below.
float clouds_altocumulus_occlusion_on_lower_layers(
    ...
    for (uint i = 0u; i < steps; ++i) {
        ...
        optical_depth += clouds_altocumulus_density(sp) * step_len;
    }

    return optical_depth;
}

#define PHOTON_ALTOCUMULUS_OCCLUSION_LOWER_DEFINED
...
CloudsResult draw_altocumulus_clouds(
    ...
    float high_coverage = linear_step(0.0, 0.0, clouds_params.l1_coverage.x);

    float extinction_coeff = mix(0.1, 0.2, day_factor)
