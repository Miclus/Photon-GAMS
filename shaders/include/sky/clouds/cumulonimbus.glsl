...
float clouds_cumulonimbus_altitude_shaping(
    float noise,
    ...
    // World cloud coverage signal. Usually represented by a 2D texture/noise
    // function
    float coverage = noise;
    // Linear height within the cloud layer
    float height = altitude_fraction <= boundary
        ? linear_step(0.0, boundary, altitude_fraction)
        : linear_step(boundary, 1.0, altitude_fraction);
    // float shape = CloudShape(1.0, height, altitude_fraction <= boundary ?
    // vec2(0.45, 1.00) : vec2(0.00, 0.50));

    float density = 1.2 * linear_step(0.53, 1.0, noise)
        * linear_step(0.5, 0.75, clouds_params.cumulonimbus_amount);

    // density -= 1.0 - CloudShape(1.0, height, shapeParams) /* * 0.5 */;

    // Carve egg shape
    // density -= smoothstep(0.75, 1.0, linear_step(boundary, 1.0,
    // altitude_fraction)) * 0.6;
    float egg = pow1d5(smoothstep(0.2, 1.1, altitude_fraction)) * 0.5;
    // float anvil = 1.0 - CloudShape(1.0, height, shapeParams);
    // float blend = linear_step(0.0, 1.0, clamp01((blend_distance.y -
    // rcp(blend_distance.x)) * blend_distance.x)); blend = mix(blend, 1.0,
    // rainStrength); blend = 1.0;
    density -= egg; // * (1.0 - blend);
    // density -= anvil * blend;

    /*if (altitude_fraction < 0.3) density = density;
    else if (altitude_fraction < 0.6) density -= linear_step(0.3, 1.5,
    altitude_fraction); else if (altitude_fraction < 0.8) density -=
    linear_step(0.2, 1.0, 1.0 - altitude_fraction);*/
...
float clouds_cumulonimbus_dist2(vec3 pos) {
    return max0(
        (length(pos.xz) - clouds_cumulonimbus_distance)
        * rcp(min(clouds_cumulonimbus_end_distance, 230000)) * 5.75
    ); // [0.0 - 5.75]
}

float clouds_cumulonimbus_density(vec3 pos) {
    const float wind_angle = CLOUDS_CUMULONIMBUS_WIND_ANGLE * degree;
    const vec2 wind_velocity = CLOUDS_CUMULONIMBUS_WIND_SPEED
        * vec2(cos(wind_angle), sin(wind_angle));

    float r = length(pos);
    if (r < clouds_cumulonimbus_radius || r > clouds_cumulonimbus_top_radius) {
        return 0.0;
    }

    float distance_fraction = linear_step(
        clouds_cumulonimbus_distance,
        clouds_cumulonimbus_end_distance,
        length(pos.xz)
    );
    float distance_fraction_start = linear_step(
        clouds_cumulonimbus_distance,
        min(clouds_cumulonimbus_distance * 4.0,
            clouds_cumulonimbus_end_distance),
        length(pos.xz)
    );
    float dist2 = clouds_cumulonimbus_dist2(pos);
    float altitude_fraction = (r - clouds_cumulonimbus_radius)
        * rcp(clouds_cumulonimbus_thickness2(dist2));
    ...
    vec4 noise
        = texture(noisetex, (0.000004 / CLOUDS_CUMULONIMBUS_SIZE) * pos.xz);

    float density = clouds_cumulonimbus_altitude_shaping(
        (max1(
             clouds_params.cumulonimbus_amount
             + max0(sqrt(min(dist2, 2.0) / 1.8) - 0.65)
         ) * noise.y
         - 0.1737 * noise.w),
        altitude_fraction,
        vec2(
            clouds_cumulonimbus_blend_distance,
            distance_fraction
        ) /*vec2(length(pos.xz), clouds_cumulonimbus_end_distance * 0.25)*/
    ); //.4 * noise.y + .07 * noise.z + .3 * noise.x + .23 * noise.w
    ...
    vec3 curl = (0.181 * CLOUDS_CUMULONIMBUS_CURL_STRENGTH)
        * texture(colortex7, 0.0002 * pos).xyz
        * smoothstep(0.4, 1.0, 1.0 - altitude_fraction);
    vec3 wind = vec3(wind_velocity * world_age, 0.0).xzy;

    // 3D worley noise for detail
    float worley_0
        = texture(colortex6, (pos + 0.2 * wind) * 0.00016 + curl * 1.0).x;
    float worley_1
        = texture(colortex6, (pos + 0.4 * wind) * 0.0010 + curl * 3.0).x;
#else
    ...
    // float detail_strength = CLOUDS_CUMULONIMBUS_DETAIL_STRENGTH * (1.00 -
    // clamp(dist2 - clouds_cumulonimbus_dist2(vec3(30000.0, 0., 0.)),
    // 0.1, 1.0));
    density -= (0.25 * CLOUDS_CUMULONIMBUS_DETAIL_STRENGTH) * sqr(worley_0)
        * dampen(clamp01(1.0 - density));
    density -= (0.20 * CLOUDS_CUMULONIMBUS_DETAIL_STRENGTH) * sqr(worley_1)
...
float clouds_cumulonimbus_optical_depth(
    ...
    float step_length
        = 0.1 * clouds_cumulonimbus_thickness / float(step_count); // m
    ...
    for (uint i = 0u; i < step_count; ++i, ray_pos += ray_step.xyz) {
        ...
        optical_depth
            += clouds_cumulonimbus_density(ray_pos + ray_step.xyz * dither)
            * ray_step.w;
    }
...
vec2 clouds_cumulonimbus_scattering(
    ...
    float scattering_integral_times_density
        = (1.0 - step_transmittance) / clouds_cumulonimbus_extinction_coeff;
    ...
    for (uint i = 0u; i < 8u; ++i) {
        scattering.x += scatter_amount
            * exp(-extinct_amount * light_optical_depth) * phase * 1.33;
        scattering.x += scatter_amount
        ...
        scatter_amount *= 0.55
            * mix(lift(
                      clamp01(clouds_cumulonimbus_scattering_coeff / 0.1),
                      0.33
                  ),
                  1.0,
                  cos_theta * 0.5 + 0.5)
...
CloudsResult draw_cumulonimbus_clouds(
    ...
    const uint primary_steps_const = CLOUDS_CUMULONIMBUS_PRIMARY_STEPS;
    ...
    vec2 sphere_dists = intersect_spherical_shell(
        air_viewer_pos,
        ray_dir,
        clouds_cumulonimbus_radius,
        clouds_cumulonimbus_top_radius
    );
    ...
    bool terrain_intersected = distance_to_terrain >= 0.0
        && r < clouds_cumulonimbus_radius
        && distance_to_terrain * CLOUDS_SCALE < dists.y;

    if (
        dists.y < 0.0 // volume not intersected
        || planet_intersected
            && r < clouds_cumulonimbus_radius // planet blocking clouds
        || terrain_intersected // terrain blocking clouds
    ) {
        return clouds_not_hit;
    }

    float xz = length(air_viewer_pos.xz);
    float in_col = smoothstep(
                       clouds_cumulonimbus_distance,
                       clouds_cumulonimbus_distance + 2000.0,
                       xz
                   )
        * (1.0
           - smoothstep(
               clouds_cumulonimbus_end_distance - 2000.0,
               clouds_cumulonimbus_end_distance,
               xz
           ));
    float immersion
        = clouds_shell_immersion(
              air_viewer_pos,
              clouds_cumulonimbus_radius,
              clouds_cumulonimbus_top_radius
          )
    ...
    uint primary_steps = primary_steps_const;
    if (in_col > 0.02) {
    ...
    for (uint i = 0u; i < primary_steps; ++i) {
        ...
        float altitude_fraction = (length(ray_pos) - clouds_cumulonimbus_radius)
            * rcp(clouds_cumulonimbus_thickness2(
                clouds_cumulonimbus_dist2(ray_pos)
            ));
        ...
        float step_optical_depth
            = density * clouds_cumulonimbus_extinction_coeff * step_length;
        ...
        float light_optical_depth = clouds_cumulonimbus_optical_depth(
            ray_pos,
            light_dir,
            hash.x,
            lighting_steps
        );
        float sky_optical_depth = clouds_cumulonimbus_optical_depth(
            ray_pos,
            sky_dir,
            hash.y,
            ambient_steps
        ...
        float ground_optical_depth
            ...
            * altitude_fraction
            * clouds_cumulonimbus_thickness2(
                  clouds_cumulonimbus_dist2(ray_pos)
            ); // guess optical depth to the ground using altitude fraction and
               // density from this sample

        scattering
            += clouds_cumulonimbus_scattering(
                   density,
                   light_optical_depth,
                   sky_optical_depth,
