...
const float clouds_cumulus_radius = planet_radius + CLOUDS_CUMULUS_ALTITUDE * 2.0;
...
const float clouds_altocumulus_radius
    = planet_radius + CLOUDS_ALTOCUMULUS_ALTITUDE * 1.5;
...
const float clouds_cumulus_congestus_distance
    = CLOUDS_CUMULUS_CONGESTUS_DISTANCE;
const float clouds_cumulus_congestus_end_distance
...
const float clouds_cumulonimbus_radius
    = planet_radius + CLOUDS_CUMULONIMBUS_ALTITUDE;
const float clouds_cumulonimbus_thickness
    = /*CLOUDS_CUMULONIMBUS_ALTITUDE*/ 1146 * CLOUDS_CUMULONIMBUS_THICKNESS;
const float clouds_cumulonimbus_top_radius = clouds_cumulonimbus_radius
    + clouds_cumulonimbus_thickness * CLOUDS_CUMULONIMBUS_END_DISTANCE
        * 0.00004;
float clouds_cumulonimbus_distance
    = mix(CLOUDS_CUMULONIMBUS_DISTANCE, 0.0, rainStrength);
...
const float clouds_cumulonimbus_extinction_coeff
    = 0.05 * CLOUDS_CUMULONIMBUS_DENSITY;
float clouds_cumulonimbus_scattering_coeff
    = clouds_cumulonimbus_extinction_coeff * (1.0 - 0.33 * rainStrength);

const float clouds_thunderhead_radius
    = planet_radius + CLOUDS_THUNDERHEAD_ALTITUDE - 1000.0;
const float clouds_thunderhead_thickness
    = CLOUDS_THUNDERHEAD_ALTITUDE * CLOUDS_THUNDERHEAD_THICKNESS;
const float clouds_thunderhead_top_radius
    = clouds_thunderhead_radius + clouds_thunderhead_thickness * 20.0;

const float clouds_towering_cumulus_radius
    = planet_radius + CLOUDS_TOWERING_CUMULUS_ALTITUDE * 2.0;
const float clouds_towering_cumulus_thickness
    = CLOUDS_TOWERING_CUMULUS_ALTITUDE * CLOUDS_TOWERING_CUMULUS_THICKNESS;
const float clouds_towering_cumulus_top_radius
    = clouds_towering_cumulus_radius + clouds_towering_cumulus_thickness * 13.0;

// GAMS Clouds - Old daily weather variables set in Settings, default use D0
// values
vec2 clouds_towering_cumulus_coverage
    = vec2(CLOUDS_TOWERING_CUMULUS_MIN, CLOUDS_TOWERING_CUMULUS_MAX);
vec2 clouds_thunderhead_coverage
    = vec2(CLOUDS_THUNDERHEAD_MIN, CLOUDS_THUNDERHEAD_MAX);
