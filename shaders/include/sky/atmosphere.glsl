...
// Darker, deeper blue for sky background only (sun/moon transmittance paths
// skip this; volumetric clouds sample scattering with
// apply_blue_sky_darkening_grade = false)
vec3 atmosphere_sky_color_grade(vec3 atmosphere) {
    return atmosphere * vec3(0.78, 0.88, 1.0) * BLUE_SKY_DARKENING;
}
...
vec3 atmosphere_scattering(
    ...
    return atmosphere_sky_color_grade(atmosphere_post_processing(scattering));
}
...
vec3 atmosphere_scattering_base(
    vec3 ray_dir,
    vec3 sun_color,
    vec3 sun_dir,
    vec3 moon_color,
    vec3 moon_dir,
    bool use_klein_nishina_phase,
    bool apply_blue_sky_darkening_grade
) {
    ...
    float mie_phase_moon
        = atmosphere_mie_phase(nu_moon, use_klein_nishina_phase);

    vec3 atmosphere
        = (scattering_sc + scattering_sm * mie_phase_sun) * sun_color
        + (scattering_mc + scattering_mm * mie_phase_moon) * moon_color;

    vec3 processed = atmosphere_post_processing(atmosphere);
    return apply_blue_sky_darkening_grade
        ? atmosphere_sky_color_grade(processed)
        : processed;
}
...
vec3 atmosphere_scattering(
    ...
    return atmosphere_scattering_base(
        ray_dir,
        sun_color,
        sun_dir,
        moon_color,
        moon_dir,
        use_klein_nishina_phase,
        apply_blue_sky_darkening_grade
    );
}

// Default: apply blue-sky grade (deeper blue) for sky background / reflections
vec3 atmosphere_scattering(
    vec3 ray_dir,
    vec3 sun_color,
    vec3 sun_dir,
